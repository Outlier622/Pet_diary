...
import 'water_log_store.dart';

class WaterLogModal extends StatefulWidget {
  const WaterLogModal({super.key});

  @override
  State<WaterLogModal> createState() => _WaterLogModalState();
}

class _WaterLogModalState extends State<WaterLogModal> {
  List<WaterLogItem> _items = [];
  bool _loading = true;
  ...
  Future<void> _init() async {
    final items = await WaterLogStore.load();
    if (!mounted) return;
    setState(() {
      _items = items;
      _loading = false;
  ...
  Future<void> _add() async {
    ...
    await WaterLogStore.saveAll(next);
  }
...
class _Draft {
  ...
  final String amount;
  final String note;
  _Draft(this.date, this.amount, this.note);
}
